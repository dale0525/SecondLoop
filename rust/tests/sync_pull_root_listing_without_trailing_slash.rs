...
struct NoSlashRootListingRemote {
    inner: sync::InMemoryRemoteStore,
    remote_root_dir: String,
    device_id: String,
}

impl NoSlashRootListingRemote {
...
impl sync::RemoteStore for NoSlashRootListingRemote {
    ...
    fn list(&self, dir: &str) -> anyhow::Result<Vec<String>> {
        // Simulate buggy PROPFIND parsing: directory entries missing the trailing "/".
...
#[test]
fn pull_accepts_root_device_dirs_without_trailing_slash() {
    ...
    // Device B pulls from a remote where root listing lacks a trailing slash for device dirs.
    let remote = NoSlashRootListingRemote::new(inner_remote, remote_root_dir, device_id_a);
    ...
    assert!(applied > 0, "expected pull to apply ops from device dirs");
}
