...
#[test]
fn ask_ai_prompt_catalog_excludes_unrelated_recent_attachments() {
    let temp_dir = tempfile::tempdir().expect("tempdir");
    let app_dir = temp_dir.path().join("secondloop");
    ...
    let relevant =
        db::insert_attachment(&conn, &key, &app_dir, b"report", "text/plain").expect("attachment");
    db::link_attachment_to_message(&conn, &key, &message.id, &relevant.sha256).expect("link");
    db::upsert_attachment_metadata(
        &conn,
        &key,
        &relevant.sha256,
        Some("Quarterly project delta report"),
        &["delta-report.txt".to_string()],
        &[],
    )
    .expect("metadata");
    db::mark_attachment_annotation_ok(
    ...
    let unrelated = db::insert_attachment(&conn, &key, &app_dir, b"grocery", "text/plain")
        .expect("unrelated attachment");
    db::upsert_attachment_metadata(
        &conn,
        &key,
        &unrelated.sha256,
        Some("Weekend grocery list"),
        &["groceries.txt".to_string()],
        &[],
    )
    ...
    rag::ask_ai_with_provider(
        ...
        &key,
        &conversation.id,
        "what does the project delta attachment say?",
        4,
        rag::Focus::ThisThread,
        &provider,
        &mut |_ev| Ok(()),
    ...
    assert!(prompt.contains("Resources catalog (attachments):"));
    assert!(prompt.contains(&format!("secondloop://attachment/{}", relevant.sha256)));
    assert!(
        !prompt.contains(&format!("secondloop://attachment/{}", unrelated.sha256)),
        "prompt should not include unrelated recent attachment catalog entry: {prompt}"
    );
}
