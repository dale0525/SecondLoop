...
#[test]
fn managed_vault_progress_paths_propagate_second_device_changes_to_third_device() {
    let (base_url, stop_tx, _state, handle) = start_mock_managed_vault_server();
    let vault_id = "v1".to_string();
    let id_token = "test_uid".to_string();
    ...
    let mut push_progress_a = Vec::new();
    let pushed_a = sync::managed_vault::push_ops_only_with_progress(
        ...
        &vault_id,
        &id_token,
        &mut |done, total| push_progress_a.push((done, total)),
    )
    .expect("push A");
    assert!(pushed_a > 0);
    assert_eq!(push_progress_a.last().copied(), Some((pushed_a, pushed_a)));
