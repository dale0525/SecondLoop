fn apply_op(conn: &Connection, db_key: &[u8; 32], op: &serde_json::Value) -> Result<()> {
    ...
    match op_type {
        ...
        "secretary.memory_proposal.upsert.v1" => {
            apply_secretary_memory_proposal_upsert(conn, db_key, &op["payload"])
        }
        "secretary.memory_proposal.state.v1" => {
            apply_secretary_memory_proposal_state(conn, &op["payload"])
        }
        "knowledge.page.upsert.v1" => apply_knowledge_page_upsert(conn, db_key, &op["payload"]),
        "knowledge.page.state.v1" => apply_knowledge_page_state(conn, &op["payload"]),
        "planning.output.upsert.v1" => apply_planning_output_upsert(conn, db_key, &op["payload"]),
        "planning.output.state.v1" => apply_planning_output_state(conn, &op["payload"]),
        "secretary.run.upsert.v1" => apply_secretary_run_upsert(conn, db_key, &op["payload"]),
        "secretary.tool_call.upsert.v1" => {
            apply_secretary_tool_call_upsert(conn, db_key, &op["payload"])
        }
