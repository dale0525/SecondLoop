...
fn build_prompt_with_actions(question: &str, contexts: &[String], actions: Option<&str>) -> String {
    build_prompt_with_actions_and_history(question, contexts, actions, None, None)
}
...
fn build_prompt_with_actions_and_history(
    ...
    if let Some(resources_catalog) = resources_catalog {
        if !resources_catalog.trim().is_empty() {
            out.push_str("\nResources catalog (use these links as citations):\n");
            out.push_str(resources_catalog);
            out.push_str("\nCitation rules for attachment evidence:\n");
            out.push_str("- Every paragraph that uses attachment evidence must include at least one citation.\n");
            out.push_str("- Use markdown citations with `secondloop://attachment/<sha>`.\n");
            out.push_str("- When quoting a specific passage, use `secondloop://attachment/<sha>?kind=<kind>&chunk=<i>`.\n");
        }
    }
...
fn build_attachment_file_link(attachment_sha256: &str) -> String {
    format!("secondloop://attachment/{attachment_sha256}")
}

fn build_attachment_chunk_link(attachment_sha256: &str, kind: &str, chunk_index: i64) -> String {
    format!(
        "secondloop://attachment/{attachment_sha256}?kind={kind}&chunk={chunk_index}"
    )
}

fn normalize_resource_label(raw: &str) -> String {
    let mut label = raw.trim().replace('[', "(").replace(']', ")");
    if label.len() > 120 {
        label.truncate(120);
    }
    label
...
fn resolve_attachment_resource_label(
    conn: &Connection,
    key: &[u8; 32],
    attachment_sha256: &str,
) -> String {
    if let Ok(Some(meta)) = db::read_attachment_metadata(conn, key, attachment_sha256) {
        if let Some(title) = meta.title.as_deref().map(str::trim).filter(|s| !s.is_empty()) {
            return normalize_resource_label(title);
        }
        if let Some(filename) = meta.filenames.first().map(String::as_str) {
            if !filename.trim().is_empty() {
                return normalize_resource_label(filename);
            }
        }
        if let Some(url) = meta.source_urls.first().map(String::as_str) {
            if !url.trim().is_empty() {
                return normalize_resource_label(url);
            }
        }
    }

    let short = attachment_sha256.chars().take(8).collect::<String>();
    format!("Attachment {short}")
}

fn build_attachment_resources_catalog(
    conn: &Connection,
    key: &[u8; 32],
    hits: &[db::SimilarAttachmentChunk],
) -> Result<Option<String>> {
    const MAX_RESOURCES: usize = 20;
    const RECENT_FALLBACK: usize = 10;

    let mut lines = Vec::<String>::new();
    let mut seen_file_links = std::collections::HashSet::<String>::new();
    let mut seen_chunk_links = std::collections::HashSet::<String>::new();
    ...
    for hit in hits {
        ...
        let label = resolve_attachment_resource_label(conn, key, &hit.attachment_sha256);
        let file_link = build_attachment_file_link(&hit.attachment_sha256);
        if seen_file_links.insert(file_link.clone()) {
            lines.push(format!("- [{}]({})", label, file_link));
            if lines.len() >= MAX_RESOURCES {
                break;
            }
        }
        ...
        if seen_chunk_links.insert(chunk_link.clone()) {
            let chunk_label = format!("{} chunk {}", label, hit.chunk_index);
            lines.push(format!("- [{}]({})", chunk_label, chunk_link));
        }
    }

    if lines.len() < MAX_RESOURCES {
        for sha in db::list_recent_indexed_attachment_shas(conn, RECENT_FALLBACK)? {
            let file_link = build_attachment_file_link(&sha);
            if !seen_file_links.insert(file_link.clone()) {
                continue;
            }
            let label = resolve_attachment_resource_label(conn, key, &sha);
            lines.push(format!("- [{}]({})", label, file_link));
            if lines.len() >= MAX_RESOURCES {
                break;
            }
