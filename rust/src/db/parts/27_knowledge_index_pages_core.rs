...
struct StoredKnowledgePageRow {
    ...
    state_before_answer_muted: Option<crate::knowledge::KnowledgePageState>,
...
fn decode_stored_knowledge_page_sql_row(row: StoredKnowledgePageSqlRow) -> Result<StoredKnowledgePageRow> {
    Ok(StoredKnowledgePageRow {
        ...
        state_before_answer_muted: row.3.map(decode_page_state).transpose()?,
        default_allowed: row.4 != 0,
        requires_temporal_framing: row.5 != 0,
        confidence_level: row.6,
        source_count: row.7,
        conflict_count: row.8,
        created_at_ms: row.9,
        updated_at_ms: row.10,
        last_used_at_ms: row.11,
        human_corrected: row.12 != 0,
        tags: decode_string_list(row.13)?,
        primary_evidence_ids: decode_string_list(row.14)?,
        related_page_ids: decode_string_list(row.15)?,
        source_document_ids: decode_string_list(row.16)?,
        claim_ids: decode_string_list(row.17)?,
        compiled_title_blob: row.18,
        compiled_summary_blob: row.19,
        compiled_body_blob: row.20,
        manual_title_blob: row.21,
        manual_summary_blob: row.22,
        manual_body_blob: row.23,
    })
}
...
fn encode_optional_page_state(
    state: Option<crate::knowledge::KnowledgePageState>,
) -> Result<Option<String>> {
    state.map(encode_page_state).transpose()
}
