...
pub fn export_migration_archive_with_callbacks(
    ...
    let export_result = (|| -> Result<MigrationArchiveManifest> {
        migration_archive_record_progress(app_dir, on_event, "export", "collecting", 1, total_steps, "in_progress")?;
        ...
        migration_archive_record_progress(app_dir, on_event, "export", "writing_markdown", 2, total_steps, "in_progress")?;
        ...
        migration_archive_record_progress(app_dir, on_event, "export", "copying_attachments", 3, total_steps, "in_progress")?;
...
fn migration_archive_restore_message(
    ...
    if !item.tags.is_empty() {
        ...
        for tag_name in &item.tags {
            let tag = upsert_tag(conn, key, tag_name)?;
            tag_ids.push(tag.id);
        }
        let _ = set_message_tags(conn, key, &item.id, &tag_ids)?;
    }
    Ok(())
}
...
fn migration_archive_replace_vault_with_archive(
    app_dir: &Path,
    key: &[u8; 32],
    archive_path: &Path,
) -> Result<MigrationArchiveManifest> {
    let mut on_event = |_progress: MigrationArchiveProgress| {};
    migration_archive_replace_vault_with_archive_with_callbacks(
        app_dir,
        key,
        archive_path,
        &mut on_event,
...
fn migration_archive_replace_vault_with_archive_with_callbacks(
    ...
    let result = migration_archive_restore_from_materialized_source_with_callbacks(
        app_dir,
        key,
        &source.root_dir,
        on_event,
...
pub fn import_migration_archive_with_callbacks(
    ...
    let snapshot_path = if estimate.item_count > 0 || estimate.attachment_count > 0 {
        ...
        let path = snapshot_dir.join(format!("{}.zip", uuid::Uuid::new_v4()));
        let conn = open(app_dir)?;
        export_migration_archive(&conn, key, app_dir, &path)?;
        Some(path)
    } else {
        None
    };
    ...
    match migration_archive_replace_vault_with_archive_with_callbacks(app_dir, key, archive_path, on_event) {
        ...
        Err(err) => {
            ...
            let rollback_result = if let Some(path) = snapshot_path.as_ref() {
                migration_archive_replace_vault_with_archive(app_dir, key, path).map(|_| ())
            } else {
                Ok(())
            };
