...
public:
  ...
  JustAudioEventSink::JustAudioEventSink(flutter::BinaryMessenger* messenger, const std::string& id) {
    ...
    auto event_handler = std::make_unique<flutter::StreamHandlerFunctions<>>(
      ...
      std::lock_guard<std::mutex> guard(self->sink_mutex_);
  ...
  void Success(const EncodableValue& event) {
    if (std::this_thread::get_id() == platform_thread_id_) {
    ...
    if (dispatcher_) {
      ...
      const bool enqueued = dispatcher_.TryEnqueue([self = this, event_copy = std::move(event_copy)]() mutable {
        std::lock_guard<std::mutex> guard(self->sink_mutex_);
        if (self->sink) {
          self->sink->Success(event_copy);
        }
      });
      if (enqueued) {
        return;
    ...
    std::cerr << "[just_audio_windows] Drop cross-thread success event: dispatcher unavailable" << std::endl;
  }
  ...
  void Error(const std::string& error_code,
    ...
    if (dispatcher_) {
      const bool enqueued = dispatcher_.TryEnqueue([self = this,
        error_code,
    ...
    std::cerr << "[just_audio_windows] Drop cross-thread error event: dispatcher unavailable" << std::endl;
  }
private:
  bool ensureDispatcherQueue() {
    ...
    ABI::Windows::System::IDispatcherQueueController* raw_controller = nullptr;
    const HRESULT hr = CreateDispatcherQueueController(options, &raw_controller);
    if (FAILED(hr) || raw_controller == nullptr) {
      return false;
    }

    dispatcher_controller_ = { raw_controller, winrt::take_ownership_from_abi };
    dispatcher_ = dispatcher_controller_.DispatcherQueue();
    return dispatcher_ != nullptr;
  }

  winrt::Windows::System::DispatcherQueueController dispatcher_controller_{ nullptr };
