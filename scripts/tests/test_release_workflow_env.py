...
"""
    ...
    def test_windows_velopack_script_accepts_legacy_and_channel_specific_nupkg_names(self) -> None:
        script_text = (Path(__file__).resolve().parents[2] / "scripts/package_windows_velopack.ps1").read_text(encoding="utf-8")

        self.assertIn("function Find-ExpectedVelopackPackage", script_text)
        self.assertIn('$channelPackageName = "$PackId-$resolvedVersion-$Channel-full.nupkg"', script_text)
        self.assertIn('$legacyPackageName = "$PackId-$resolvedVersion-full.nupkg"', script_text)
