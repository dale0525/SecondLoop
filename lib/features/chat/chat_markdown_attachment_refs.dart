...
final RegExp kMarkdownImageRefPattern = RegExp(
  r'!\[[^\]]*\]\((<[^>]+>|[^)\s]+)(?:\s+"[^"]*")?\)',
);
...
String rewriteDraftMarkdownImageRefs(
  ...
  return markdown.replaceAllMapped(kMarkdownImageRefPattern, (match) {
...
Set<String> collectPersistedMarkdownAttachmentShas(String markdown) {
  ...
  for (final match in kMarkdownImageRefPattern.allMatches(markdown)) {
    final sourceToken = match.group(1);
...
String unwrapMarkdownImageSourceToken(String token) {
