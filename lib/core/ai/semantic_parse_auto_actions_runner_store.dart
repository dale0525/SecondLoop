...
final class BackendSemanticParseAutoActionsStore
  ...
  Future<SemanticParseMessageInput?> getMessageInput(String messageId) async {
    ...
    final attachmentSnippets =
        await _loadAttachmentSemanticSnippets(messageId, sourceText);
  ...
  Future<List<String>> _loadAttachmentSemanticSnippets(
    String messageId,
    String sourceText,
  ) async {
  ...
  Future<bool> completeFollowupIfCurrentAttempt({
    ...
    if (dueAtMs != null) {
      await _backend.updateTodoDueWithScope(
        _sessionKey,
        todoId: todoId,
        dueAtMs: dueAtMs,
    ...
    if (newStatus != null && newStatus.trim().isNotEmpty) {
      previousStatus = await setTodoStatusFromMessage(
        messageId: messageId,
        todoId: todoId,
        newStatus: newStatus,
        expectedAttemptId: expectedAttemptId,
