...
void main() {
  testWidgets('Long message bubble is collapsed and can open full viewer',
      (tester) async {
    tester.view
    ...
    final longText = StringBuffer()
      ...
      ..writeln('**bold**')
      ..writeln()
      ..writeln(List<String>.filled(200, 'line').join(' '))
      ..writeln()
      ..writeln('TAIL_MARKER');
    ...
    final bubbleHeight =
        tester.getSize(find.byKey(const ValueKey('message_bubble_m1'))).height;
    expect(bubbleHeight, lessThanOrEqualTo(420));

    expect(find.byKey(const ValueKey('message_view_full_m1')), findsOneWidget);
    await tester.tap(find.byKey(const ValueKey('message_view_full_m1')));
    await tester.pumpAndSettle();

    expect(find.byKey(const ValueKey('message_viewer_page')), findsOneWidget);
    expect(find.text('TAIL_MARKER'), findsOneWidget);
  });
}
