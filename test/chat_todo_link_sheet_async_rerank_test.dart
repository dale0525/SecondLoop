...
void main() {
  testWidgets(
      'Todo update sheet reranks asynchronously after initial semantic timeout',
      (tester) async {
    ...
    final backend = _Backend(
      ...
      similarTodoThreads: const [
        TodoThreadMatch(todoId: 't_semantic', distance: 0.2),
      ],
    );
    ...
    expect(
      find.descendant(
        of: sheet,
        matching: find.text('下午 2 点接待客户（仅 due）'),
      ),
      findsOneWidget,
    );
    expect(
      find.descendant(
        of: sheet,
        matching: find.text('写周报（语义匹配）'),
      ),
      findsNothing,
    );

    for (var i = 0; i < 150 && find.text('写周报（语义匹配）').evaluate().isEmpty; i++) {
      await tester.pump(const Duration(milliseconds: 20));
    }
    await tester.pumpAndSettle();
...
final class _Backend extends AppBackend {
  ...
  Future<List<TodoThreadMatch>> searchSimilarTodoThreads(
    ...
    int topK = 10,
      ...
      List<TodoThreadMatch>.from(_similar);
