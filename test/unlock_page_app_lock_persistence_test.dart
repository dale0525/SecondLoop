...
void main() {
  testWidgets('unlock: auto lock off -> persists session key', (tester) async {
    ...
    await tester.enterText(find.byKey(const ValueKey('unlock_password')), 'pw');
  ...
  testWidgets('unlock: auto lock on -> does not persist session key',
      (tester) async {
    SharedPreferences.setMockInitialValues({
  ...
  testWidgets('unlock: auto lock on + biometric on -> persists session key',
      (tester) async {
    SharedPreferences.setMockInitialValues({
      'app_lock_enabled_v1': true,
