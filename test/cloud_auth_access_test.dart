...
import 'package:secondloop/core/cloud/cloud_auth_access.dart';
...
void main() {
  test('interactive cloud auth token loads persisted session', () async {
  ...
  test('background cloud auth token keeps persisted session untouched',
      () async {
    final previous = debugDefaultTargetPlatformOverride;
    addTearDown(() {
      debugDefaultTargetPlatformOverride = previous;
  ...
  test('best-effort warm uses interactive token path', () async {
    ...
    await bestEffortWarmCloudAuth(controller);
