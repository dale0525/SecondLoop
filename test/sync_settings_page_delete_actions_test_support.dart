...
Future<void> _scrollChatAwayFromBottom(WidgetTester tester) async {
  ...
  for (final offset in const <Offset>[
    ...
    await tester.pumpAndSettle();
    if (find
        .byKey(const ValueKey('chat_jump_to_latest'))
        .evaluate()
        .isNotEmpty) {
      return;
    }
...
final class _ChatDeleteActionsBackend extends _DeleteActionsBackend
    implements SecretaryBackend {
  _ChatDeleteActionsBackend()
      : _messages = <Message>[
          for (var i = 1; i <= 79; i++)
            Message(
              id: 'm$i',
  ...
  Completer<void>? _pendingMemoryProposalCreate;

  void completePendingMemoryProposalCreate() {
    final pending = _pendingMemoryProposalCreate;
  ...
  @override
  Future<void> resetVaultDataPreservingLlmProfiles(Uint8List key) async {
    await super.resetVaultDataPreservingLlmProfiles(key);
    _messages.clear();
    _proposals.clear();
  }
  ...
  Future<SecretaryMemoryProposalRecord> createSecretaryMemoryProposal(
    ...
    if (sourceMessageId == 'memory_source') {
      final pending = _pendingMemoryProposalCreate ??= Completer<void>();
