...
Use `pixi run release` (no positional version) to run preflight + AI bump decision + automatic tag publish. Local runs can read release env vars from `.env.local`. For troubleshooting-only rehearsal without local LLM calls, use `pixi run release --dry-run --skip-llm` (GitHub Actions still performs full checks on real release tags).
